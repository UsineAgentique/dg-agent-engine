def process_dg_mission(channel_id: str, channel_type: str, user_text: str):
    ...
    try:
        ...
        # Boucle d'exécution dynamique des outils (permet au modèle d'enchaîner ou de s'arrêter proprement)
        for _ in range(max_turns):
            ...
            # Si le modèle ne demande aucun outil, on sort de la boucle pour rédiger la réponse
            ...
            # Sécurité anti-boucle infinie : si le max de tours est atteint, on force un dernier appel sans outils
            final_forced = groq_client.chat.completions.create(
                model=model_name,
                messages=messages,
                tool_choice="none",
                temperature=0.1
            )
            response_message = final_forced.choices[0].message
            messages.append(response_message)

        draft_response = response_message.content.strip() if response_message and response_message.content else "Directive exécutée."

        # ÉTAPE DE DOUBLE VÉRIFICATION (Contrôle interne des risques & hallucinations)
        audit_prompt = (
